...
import local_ocpa.ocpa.algo.predictive_monitoring.obj
...
# create graph dataset & labels for remaining time regression
import torch  # torch au lieu de tensorflow
...
def generate_graph_dataset(feature_graph_list, indices, ocel_object, k, target, include_last=True):
    ...
    for gid in tqdm(idx):
        ...
        dgl_graph.ndata['event_indices'] = torch.tensor(event_indices)  # Modification here

        dgl_graph.ndata['k'] = torch.tensor(
            [k for i in range(0, len(event_indices))], dtype=torch.int64)  # Modification here
        dgl_graph.ndata['features'] = torch.tensor(features, dtype=torch.float32)  # Modification here
        dgl_graph.ndata[target[0]] = torch.tensor(labels, dtype=torch.float32)  # Modification here
        # extract subgraph and label for each node set as terminal node
        correct = 0
        if not include_last:
            correct = 1
...
# // ------------------------------------------------ //
# Original code of `generate_graph_dataset` method. 
# problems with tensorflow and dgl compatibility, it's working with pytorch for Bayesian network prediction
# Comment by : @paulTaurand
# // ------------------------------------------------ //
