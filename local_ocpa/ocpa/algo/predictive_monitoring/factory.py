...
import local_ocpa.ocpa.algo.predictive_monitoring.event_based_features.extraction_functions as event_features
import local_ocpa.ocpa.algo.predictive_monitoring.execution_based_features.extraction_functions as execution_features
from local_ocpa.ocpa.algo.predictive_monitoring.obj import Feature_Storage
...
def apply(ocel, event_based_features=[], execution_based_features=[], event_attributes=[], event_object_attributes=[], execution_object_attributes=[], workers = 4, multi_output_event_features = []):
    ...
    nodes_time = 0
    adding_time = 0
    # for c_id in range(0,len(ocel.proces_executions)):
    #     case = ocel.process_executions[c_id]
    #     s_time = time.time()
    #     case_graph = ocel.graph.eog.subgraph(case)
    #     feature_graph = Feature_Storage.Feature_Graph(
